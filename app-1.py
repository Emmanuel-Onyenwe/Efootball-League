...
def sync_matchdays():
    """
    Non-destructive reconciliation: assigns/repairs the `matchday` number on
    EVERY existing Match row (approved, submitted, AND pending) so the whole
    90-match season lines up with a real Circle Method schedule, without ever
    deleting or duplicating a match that's already been played.

    Fixes two bugs found in the previous /rebuild-matchdays route:
      1. It queried a `Player` model that doesn't exist in this codebase
         (only `User` does) - that route could never run past that line.
      2. It checked for status == 'completed', but this app marks a verified
         match 'approved'. So even with the Player bug fixed, it would never
         have recognized Matchday 1 as already played, and would have wiped
         pending matches then recreated fixtures for already-approved pairs
         too - creating duplicates instead of syncing.
    """
    if current_user.role != 'admin':
        flash("Access Denied: Admins only.", "error")
        return redirect(url_for('index'))
    ...
    try:
        ...
        db.session.rollback()  # Ignore if the column already exists
    ...
    # Canonical matchday for each pair's Leg 1 and Leg 2 meeting, keyed by
    # the unordered pair - we assign matchdays by MEETING ORDER, not by
    # literal home/away identity, since a match already played may not have
    # been played in the schedule's "expected" home/away direction.
    pair_matchdays = {}  # {pair: [leg1_matchday, leg2_matchday]}
    for matchday_index, round_matches in enumerate(schedule, start=1):
        for home_id, away_id in round_matches:
            pair = tuple(sorted([home_id, away_id]))
            pair_matchdays.setdefault(pair, []).append(matchday_index)

    # Existing rows per pair, oldest first = the order they were actually
    # created/played in - this preserves real match history untouched.
    ...
    for pair, matchdays in pair_matchdays.items():
        ...
        for leg_index, matchday in enumerate(matchdays):
            if leg_index < len(existing):
                # A match for this leg already exists (any status) - just
                # correct its matchday number, don't touch its result.
                m = existing[leg_index]
                if m.matchday != matchday:
                    m.matchday = matchday
                    updated += 1
            else:
                # No row for this leg yet - create it using the schedule's
                # intended home/away for that matchday.
