...
def generate_fixtures():
    ...
    # Email every active player their personal fixture list now that IDs exist
    for player in users:
        ...
        for m in player_matches:
            opponent = m.player_b.name if m.player_a_id == player.id else m.player_a.name
            due = m.deadline.strftime('%a %d %b, %H:%M UTC') if m.deadline else "TBD"
            rows += f"<tr><td>MD{m.matchday}</td><td>vs {opponent}</td><td>{due}</td></tr>"

        html_msg = f"""
        <h3>Your Panic Keh League Fixtures Are Live! 🎮</h3>
        <p>The season schedule has been generated. Here's your run of fixtures:</p>
        <table cellpadding="6" style="border-collapse:collapse;">
          <tr><th>Matchday</th><th>Fixture</th><th>Deadline</th></tr>
          {rows}
        </table>
        <p>Log in to the dashboard to submit results as you go. Good luck!</p>
        """
        try:
            send_email(player.email, "Your League Fixtures Are Live!", html_msg)
        except Exception as e:
            print(f"Fixture email to {player.email} failed: {e}")

    flash(f"Season generated: {matches_created} new fixtures created and emailed to {len(users)} players.", "success")
    return redirect(url_for('admin'))
